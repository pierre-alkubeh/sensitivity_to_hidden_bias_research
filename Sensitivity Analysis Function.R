GenMatchRose <- function(Tr, X, Y, Gamma = 5, GammaInc = 0.25, BalanceMatrix = X, estimand = "ATT", M=1, weights = NULL,
                         pop.size = 100, max.generations = 100, wait.generations = 4, hard.generation.limit = FALSE,
                         starting.values = rep(1,ncol(X)), MemoryMatrix = TRUE, exact = NULL,
  ...
  #Defines the main function to optimize for robustness. Has all inputs from GenMatch() & psens() function
  #Main function is a regular GenMatch() function except with a specialized fitness function for the optimization.
  
  rose.fitfunc <- function(matches, BM){
    #Defines a fitness function. All GenMatch fitness functions take two arguements, matches and BM (balance matrix).
    #This function will be run on every chromosome to identify its fitness value
    index.treated <- matches[,1]
    index.control <- matches[,2]
    #Gets the index position of the matched units, treatment and control.
    
    out.treat <- Y[index.treated]
    out.cont <- Y[index.control]
    #Uses the index positions to get the control and treatment outcomes of the units being matched
    
    sens <- psens(x= out.treat, y=out.cont, Gamma = Gamma, GammaInc = GammaInc)
    #Runs and assigns the rosenbaum analysis function
    len_sens <- length(sens$bounds[,1])
    #Gets number of gamma intervals based on the length of the bounds vector
    fit.value <- len_sens + sens$bounds[1,3]
    #Assigns an initial fitness value, the number of intervals plus the upper p-value of the first interval (gamma =1)
    for(i in 1:len_sens){
      if(sens$bounds[i,3] <= 0.05){
        fit.value <- len_sens - i + sens$bounds[i,3]
      }
    }
    #Runs a for-loop that assigns a new fitness value, where the last assigned value is the number of intervals,
    #minus the last statistically significant interval, plus the upper-bound p-value. 
    #Because this is a minimization algorithm, this incentivizes for the highest statistically interval and the lowest
    #upper-bound p-value at the last interval. 
    return(fit.value)
    #returns the final fitness value for the GenMatch() algorithm
  }
  
  return(GenMatch(Tr = Tr, X = X, BalanceMatrix = BalanceMatrix, estimand = estimand, M = M, weights = weights,
                  pop.size = pop.size, max.generation = max.generations, wait.generations = wait.generations,
                  hard.generation.limit = hard.generation.limit, starting.values = starting.values, fit.func = rose.fitfunc,
  ...
  #Runs and returns the GenMatch function with the new fitness function optimizing for robustness
}
#Closes function
